...
globals
  ...
  %immune
  total
  ;; what % of the population is immune

  lifespan             ;; the lifespan of a turtle
        ;; the probability of a turtle generating an offspring each tick
...
to setup-turtles
  ...
  ask n-of Outbreak-size turtles
    [ get-sick ]
  ask n-of num-doctors turtles
  [get-doctor]
  set dead 0
end

to
  get-sick ;; turtle procedure
  set sick? true
  set remaining-immunity 0
end
...
to infect ;; turtle procedure
  ask other turtles-here with [ not sick? and not doctor? ]
...
to treatment
  ask turtles with [doctor?]
  [
  ask other turtles-here with [not doctor? ]
    [get-healthy]
  ]
...
to reproduce
  ask turtles with [not doctor? and not sick?]
  [
  if (count turtles) < carrying-capacity and (random lifespan) < num-children
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] ]]
